...
指標設定・指標セット呼び出し・スコア内訳は、それぞれ **1 つのオープンボタン** を持ち、希望表示ラジオ（モードレス／別ウィンドウ）は **チャート分析画面で1つ** とする。

- 親画面のラジオは希望モードのみ更新し、**いま開いているいずれのウィンドウも変えない**
- ボタン再押下で希望モードに開く／同モードなら閉じる／開いていて希望が異なれば切替
- 各ウィンドウ内でも同じラジオで即時切替でき、**その窓の表示だけ**を変える（他窓・親の希望モードは変えない）
- 各窓は独立に開閉と表示形態（モードレス／別ウィンドウ）を記憶する
- 基準日は From / To と同様の日付入力でも選べる（チャートクリックも維持）。バー日付と一致しない入力は直前のバーへスナップ
- 週足は従来どおり `interval=1w`（日足集約 → 指標・トレンドスコアも週バーで計算）
...
- [v0.2.0 Phase 7](../roadmap/v0/v0.2/v0.2.0.md)
