...
flowchart TB
  ...
  p5[Phase5_IndicatorSets]
  p6[Phase6_ScoreBreakdown]
  p1 --> p2 --> p3 --> p4 --> p5 --> p6
```
...
- スコア用グループは指標あたり 1 つ（MACD / 一目 / RSI / CCI の二重計上をしない）
...
## Phase 5 — テクニカル指標セットの保存と呼び出し

チャート分析の指標トグルを名前付きセットとして保存し、独立ウィンドウから検索・呼び出せるようにする。

- 保存対象はカタログ ID の ON/OFF のみ（パラメータ編集は対象外）
- 保存 UI は現行の指標設定ウィンドウ（モードレス / 別ウィンドウ）内に置く
- 呼び出しはチャート分析内の独立ウィンドウ。指標設定と同時に開ける
- 検索はセット名の部分一致と、チェックした指標をすべて含む AND フィルタ
- 呼び出すと現行の `enabledIds` に反映し、開いている指標設定ウィンドウとチャートが同期する
- ユーザー所有で PostgreSQL に保存する（localStorage にはしない）
- 可読性のためのコメント
- 単体テスト（カバレッジ 100%）
- 詳細は [ADR 008](../../../adr/008-indicator-sets.md)

対象 Issue: [GitHub #14](https://github.com/KenichiroArai/market-platform/issues/14)

## Phase 6 — チャート分析スコアの内訳

トレンドスコアはそのままに、基準日ごとのグループ／個別内訳をモードレスウィンドウで確認できるようにする。

- 総合スコア表示は基準日連動（未選択時は直近の有効バー）。チャートクリックで基準日をセットする
- 「スコア内訳」ボタンからモードレスウィンドウを開き、グループ寄与と個別指標点数を表示する
- 内訳のグループ分けは既存の `scoreGroup`（採点正本）に合わせる。新規 API は作らない
- MACD / RSI のカタログ `categories` 重複を削除し、`scoreGroup` に揃える（MACD=`trend`、RSI=`oscillator`）
- 可読性のためのコメント
- 単体テスト（カバレッジ 100%）
- 詳細は [ADR 007](../../../adr/007-trend-score.md)

対象 Issue: [GitHub #15](https://github.com/KenichiroArai/market-platform/issues/15)

## Phase 7 — チャート分析画面の整理

チャート分析のウィンドウ出し分けと基準日操作を整理し、週足経路を維持する。

- 指標設定・指標セット呼び出し・スコア内訳を各1ボタンにし、希望表示ラジオ（モードレス／別ウィンドウ）は画面で1つ
- 親のラジオは開いている表示を変えず、ボタン再押下で反映。ウィンドウ内切替はその窓だけ即時変更（各窓は独立に状態を記憶）
- スコア内訳も別ウィンドウ対応
- 基準日を From / To と同様の日付入力で選べるようにする（チャートクリックも維持。バー日付へスナップ）
- 週足（`interval=1w`）で価格・指標・トレンドスコアが週バー基準になることを確認・回帰テスト
- 可読性のためのコメント
- 単体テスト（カバレッジ 100%）
- 詳細は [ADR 005](../../../adr/005-chart-analysis.md) / [ADR 007](../../../adr/007-trend-score.md)

対象 Issue: [GitHub #16](https://github.com/KenichiroArai/market-platform/issues/16)
