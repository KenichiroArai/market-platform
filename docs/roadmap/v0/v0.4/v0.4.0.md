# 開発ロードマップ（v0.4.0）

実行履歴（`BacktestRun`）の操作性を高め、指標セットの細かい設定とバックテスト結果の分析しやすさを整える版です。

```mermaid
flowchart TB
  p1[Phase1_RunHistorySearchDelete]
  p2[Phase2_IndicatorSetConfig]
  p3[Phase3_BacktestResultAnalysis]
  p4[Phase4_MoneyManagement]
  p1 --> p2
  p2 --> p3
  p3 --> p4
```

## 共通品質要件（全 Phase）

各 Phase の実装では、次を満たすこと。

- **可読性のためのコメント**: モジュール・公開 API・分岐・外部 I/O・非自明な意図を日本語で厚めに記載する
- **単体テスト（カバレッジ 100%）**: statements / branches / functions / lines（analysis は pytest-cov）

## Phase 1 — 実行履歴の操作性向上

実行履歴を条件付けて検索し、論理削除できるようにする。

- `BacktestRun.isActive`（論理削除。`false` で一覧デフォルトから除外）
- `GET /backtests` — 条件付き一覧（軽量 DTO。trades / equityPoints 不含）
- `GET /backtests/:id` — 詳細（従来どおり trades / equityPoints 込み）
- `DELETE /backtests/:id` — 単一論理削除
- `DELETE /backtests` — 検索条件と同一クエリでの一括論理削除（`{ deletedCount }`）
- 検索条件: 銘柄 / 戦略 / 指標セット / 検証期間 / 実行日時 / 削除済みを含む（`isActive=all`）
- デフォルト: 活動中のみ（`isActive=true`）
- UI: `/backtests` 結果タブの「検索…」→ `ModelessWindow` パネル
- 削除: 行ごと / 検索結果一括 / 活動中すべて
- 詳細は [ADR 013](../../../adr/013-backtest-run-history-management.md)
- 対象 Issue: [GitHub #27](https://github.com/KenichiroArai/market-platform/issues/27)

## Phase 2 — テクニカル指標の細かい設定

指標セットにパラメータ上書き・スコア配点・買い売り閾値を持たせ、保存・上書き・複製を可能にする。

- `IndicatorSet` に `indicatorParams` / `groupWeights` / `buyThreshold` / `sellThreshold` を追加
- `PATCH /indicator-sets/:id` — 上書き更新
- `POST /indicator-sets` — 新フィールド対応（同名は 409、クライアントが確認後 PATCH）
- `GET /symbols/:id/indicators` — オプション `indicatorParams`（JSON）
- `GET /symbols/:id/trend-score` — オプション `groupWeights`（JSON）
- グループ配点は合計 100% 必須（不一致は登録不可）
- UI: `/charts` 指標設定でパラメータ・配点・閾値を編集
- UI: セット呼び出しに複製ボタン（指標設定へ遷移、元名をセット名欄に表示）
- UI: 同名保存時に上書き確認ダイアログ
- バックテスト `trendScore` モードはセット保存済み閾値をデフォルト使用
- 詳細は [ADR 014](../../../adr/014-indicator-set-config.md)
- 対象 Issue: [GitHub #28](https://github.com/KenichiroArai/market-platform/issues/28)

## Phase 3 — バックテスト結果の分析しやすさ

バックテスト結果を分析しやすくする（Issue #29）。

- 実行期間の初期値をチャート分析と共有（`defaultChartFromDate` / `defaultChartToDate`）
- エクイティカーブ見出し横にホバー説明（HoverHelp）
- 取引履歴に総合スコア列＋トレンドスコア時はグループ寄与列・採点対象指標のエントリー／エグジット列（`entryScoreBreakdown` / `exitScoreBreakdown`）
- 日次データタブ（サマリー＋equity × 価格明細。`scoreBreakdown` のグループ／指標列）
- 日次詳細 ZIP（`daily_detail.csv` + summary/trades/prices 補足）。クライアント生成（fflate）。導線は日次データタブ
- Equity に `decisionScore` / `scoreBreakdown`、Trade にスコア内訳 Json を永続化
- 詳細は [ADR 015](../../../adr/015-backtest-result-analysis.md)
- 対象 Issue: [GitHub #29](https://github.com/KenichiroArai/market-platform/issues/29)

## Phase 4 — 資金管理（マネーマネージメント）

タートルズ系の資金管理をシグナルから独立して導入する（Issue #30）。

- Analysis `money_management/`（PositionSizing / Risk / Pyramiding / Drawdown / Correlation / Fees）
- ボラティリティ基準のポジションサイズ、ATR ストップ、ピラミッディング、ドローダウン時リスク縮小
- 相関グループ（単一銘柄向けキャップ。横断制限は将来課題）
- 売買方針 `longOnly` | `longShort`
- 手数料: レートまたは固定額（0 可）、数量の整数／小数切替
- UI: 開始資金横の ModelessWindow（タブ）、結果の MM 列・統計、履歴から設定復元
- 詳細は [ADR 016](../../../adr/016-money-management.md)
- 対象 Issue: [GitHub #30](https://github.com/KenichiroArai/market-platform/issues/30)
